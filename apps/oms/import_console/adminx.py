            ...
            if self.modify_models_batch:
                ...
                for obj in queryset:
                    ...
                    if q_goods.exists():
                        ...
                        self.message_user("单号%s货品错误，查看系统是否有此货品" % obj.order_id, "error")
                        n -= 1
                        obj.mistake_tag = 4
                        obj.save()
                        continue
                    ...
                    if pre_purchase.exists():
                        ...
                        self.message_user("单号%s采购单号错误，查看系统是否有此采购单" % obj.order_id, "error")
                        n -= 1
                        obj.mistake_tag = 6
                        obj.save()
                        continue
                    ...
                    if q_supplier.exists():
                        ...
                        self.message_user("单号%s工厂错误，查看系统是否有此工厂" % obj.order_id, "error")
                        n -= 1
                        obj.mistake_tag = 3
                        obj.save()
                        continue
                    # 入库单号，货品，仓库，采购单号和批次一样，就合并入库单，只要这四个有一个不同，则认为是独立的入库单。
                    repeat_queryset = StockInInfo.objects.filter(order_id=obj.order_id,
                    ...
                    if repeat_queryset.exists():
                        ...
                        if repeat_order.quantity_receivable >= current_quantity:
                            ...
                            repeat_order.save()
                            self.message_user("单号%s，合并货品%s" % (obj.order_id, obj.goods_id), "info")
                            obj.mistake_tag = 1
                            obj.order_status = 2
                            obj.save()
                            continue
                        else:
                            self.message_user("单号%s，重复或者单据被篡改。货品是%s" % (obj.order_id, obj.goods_id), "error")
                            obj.mistake_tag = 2
                            obj.order_status = 2
                            obj.save()
                            continue
                    # 设置入库单的入库数量。
                    _quantity_receivable = OriStockInInfo.objects.filter(order_id=obj.order_id, goods_id=obj.goods_id, warehouse=obj.warehouse, expiry_date=obj.expiry_date, order_status__in=[1, 2])
                    ...
                    fields_list = ['order_category','to_organization','order_creator','supplier_address','create_date','seller','bs_category','order_id','last_modifier','payee','stockin_time','last_modify_time','consignee','is_cancel','purchaser','status','demander','goods_id','goods_size','goods_unit','quantity_receivable','quantity_received','batch_number','expiry_date','produce_time','memorandum','origin_order_category','origin_order_id','payable_quantity','assist_quantity','multiple','price','storage']
                    ...
                    obj.order_status = 2
                    obj.save()
                    self.message_user("%s 递交完毕" % obj.order_id, "info")
...
class OriStockInUnhandleAdmin(object):
    list_display = ['order_id', 'mistake_tag', 'order_status', 'supplier', 'create_date', 'purchaser', 'goods_id',
...
class OriStockInInfoAdmin(object):
    list_display = ['order_id', 'mistake_tag', 'order_status', 'supplier', 'create_date', 'purchaser',
                    'goods_name', 'goods_size', 'goods_unit', 'quantity_receivable', 'quantity_received',
...
class OriPurchaseUnhandleAdmin(object):
    list_display = ['order_id', 'mistake_tag', 'order_status', 'purchase_time', 'delivery_date' ,'supplier','puchaser','goods_id','goods_name','quantity', 'goods_unit']
    ...
    def save_resources(self, resource):
        ...
        for row in resource:
            ...
            # 如果订单号查询，已经存在，丢弃订单，计数为重复订单
            if OriPurchaseUnhandle.objects.filter(order_id=order_id, goods_id=goods_id, price=price).exists():
                report_dic["repeated"] += 1
                report_dic["error"].append('%s单据重复导入货品为：%s' % (order_id, goods_id))
                continue
    ...
    def queryset(self):
        queryset = super(OriPurchaseUnhandle, self).queryset()
        queryset = queryset.filter(is_delete=0, order_status=1)
        return queryset
...
class OriPurchaseInfoAdmin(object):
    list_display = ['order_id', 'purchase_time', 'delivery_date' ,'supplier','puchaser','goods_id','goods_name','quantity', 'goods_unit', 'status',]
